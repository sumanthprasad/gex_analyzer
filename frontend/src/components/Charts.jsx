...
function Charts({ data, gammaWallStrike }) {
  ...
  // build calls vs puts data for Dealer Vanna
  const vannaData =
    Array.isArray(data.dealer_vanna_calls) &&
    Array.isArray(data.dealer_vanna_puts)
      ? data.dealer_vanna_calls.map((c, i) => ({
          strike: c.strike,
          calls: c.value,
          puts: data.dealer_vanna_puts[i]?.value || 0,
        }))
      : [];
  ...
  return (
    <div style={{ padding: "10px", maxWidth: "1000px", margin: "auto" }}>
      ...
      {visibleCharts.vanna && (
        <div style={forcedWrapperStyle}>
          <h3 style={chartTitleStyle}>Dealer Vanna Exposure (Calls vs Puts)</h3>
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={vannaData} margin={chartMargin} barCategoryGap="20%">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis {...commonXAxisProps} padding={{ left: 20, right: 20 }} />
              <YAxis
                tickFormatter={(val) => Number(val).toExponential(2)}
              ...
              {renderSpotLine()}
              {renderGammaWall()}
              <Bar dataKey="calls" name="Calls Vanna" fill="green" barSize={12} />
              <Bar dataKey="puts"  name="Puts Vanna"  fill="red"   barSize={12} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
